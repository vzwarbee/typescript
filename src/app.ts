...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btnCancel.addEventListener("click", () => {
    ...
    window.location = "as/assigment1/pages/game.html" as Location | (string & Location);
  });
  type Card = {
    name: string;
    img: string;
  ...
  const checkForMatch: () => void = () => {
    ...
    if (first.name === second.name) {
      alert("Found Match");
      cards[first.id].setAttribute("src", "../public/img/white.png");
      cards[second.id].setAttribute("src", "../public/img/white.png");
